...
void c15Puzzle_main(){
    ...
    Board* creatingNewBoard = createBoard(rows,cols);
	
	initializeBoard(creatingNewBoard,(int*)table,rows,cols);
		
	for(i=0;i<rows;i++)
		for(j=0;j<cols;j++)
			printf(" %d ",(creatingNewBoard->table)[i][j]);
	
	clearAndRedrawScreen(creatingNewBoard);
	printUser(user);
	...
	do{
		...
		choice = takeInput();
		playMove(creatingNewBoard,user,choice);
		
		check = checkBoard(creatingNewBoard);
