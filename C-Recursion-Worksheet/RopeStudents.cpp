...
void findBallRecursive(struct student* leader, int* min, int* max,int count){


	count++;
	
	if (leader->first != NULL)

		findBallRecursive(leader->first, min, max, count);
	...
	if (leader->second != NULL)

		findBallRecursive(leader->second, min, max, count);
	...
	if (leader->third != NULL)

		findBallRecursive(leader->third, min, max, count);
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance){
	...
	*maxBallDistance = 0;
	...
	struct student* temp = leaderStudent;

	findBallRecursive(temp, &min, &max, count);

	*minBallDistance = min;
	*maxBallDistance = max;

	if (*minBallDistance == 10000)
		*minBallDistance = 1;
